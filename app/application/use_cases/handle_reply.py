...
async def handle_user_reply(
    ...
    task_state = await db.get_task_state(issue_number, repo_url)

    if not task_state:
        logger.warning("no_active_task_for_issue", issue_number=issue_number, repo_url=repo_url)
        return

    if task_state.status not in (TaskStatus.RUNNING, TaskStatus.WAITING_REPLY):
        logger.warning(
            "task_not_awaiting_reply",
    ...
    if not task_state.active_port or not task_state.session_id:
        logger.warning("no_active_session_for_issue", issue_number=issue_number, repo_url=repo_url)
        return
    ...
    await telegram.send_message(f"Reply sent to Agent for Issue #{issue_number} in {repo_url}")
